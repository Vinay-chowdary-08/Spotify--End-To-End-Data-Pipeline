...
def songs(data):
    ...
    for row in data["items"]:
        ...
        song_element = {"song_id" : song_id, "song_name" : song_name, "song_duration" : song_duration, "url" : song_url, "popularity" :  song_popularity, "song_added" : song_added, "album_id" : album_id, "aritst_id": artist_id }
...
def lambda_handler(event, context):
    s3 = boto3.client('s3')
    bucket_name = 'spotify-etl-project-vinay-chowdary'
    key = 'raw_data/to_processed'


    spotify_data = []
    spotify_keys = []
    for file in s3.list_objects_v2(Bucket=bucket_name, Prefix=key)['Contents']:
    ...
    for data in spotify_data:
        ...
        album_df.loc[:,"release_date"] = pd.to_datetime(album_df["release_date"],format = "mixed" ,errors = "coerce")
